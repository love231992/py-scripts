...
# ************************************DPR creation starts****************************************
    def daily_report(self):
        ...
        customers1 = [20,31,28,27,17,46,13,15,14,37,100125]
...
#  program for sumifs and countifs for customers2 and appending data to Dpr
        ...
        ws.range('E19').options(transpose=True).value = count_list2
        ws.range('F19').options(transpose=True).value = sum_list2
...
#  program for sumifs and countifs for customers3 and appending data to Dpr
        ...
        ws.range('F25').options(transpose=True).value = sum_list3
        ws.range('E25').options(transpose=True).value = count_list3
        # print(sum_list3)
        # print(count_list3)
...
#                   *********************  Balance starts **********************************
    def balance(self):
        ...
        path = r'\\10.9.32.2\adm\Ash\FY 2019-20\Sale detail sheet'
        # path = r'\\10.9.32.2\adm\Ash\FY 2019-20\Sale detail sheet'
        userpath = f'SALE DETAIL SHEET {month.upper()} 2019' 
...
# **************************************** Sale QTy Start *************************************************
    def sale_detail(self):
        ...
        if userpath =='' :
            ...
            userpath = userpath
        abc = os.path.join(path, userpath+'.xlsx')
        customers1 = [20,31,28,27,17,46,13,15,14,37,100125,100051,100062,100087,100071,100070,100057,100056,100066,100068,100086,100091,100103,100126,100131,100145,100150,100152,100140,100165,100180]
        customers2 = [ 'Asian Cements','Asian Fine', 'UTCL Roorkee', 'UTCL Bagheri','UTCL Panipat','UTCL Sikandrabad','Ambuja Nalagargh','Ambuja Ropar','Ambuja Roorkee','Ambuja Dadri','ACC LTD','Fateh','Everest','Hemkund sahib','Amritsaria','Jai Shiv shankar','Ramjee','Paras','Manju','Sachin','R.S.Green','BTS','S.A.Bricks','Royal','M.M. Concrete','Fairmont','Aniket','A One','ONS','NTC','Guru Teg Bhadar']
